...
우선 요구사항을 “회원이 선호 음식 카테고리로 설정한 식당 목록을 평점순으로 20개까지 조회”로 정하였습니다.

우선 회원 id를 통해 회원-음식 카테고리에서 음식 카테고리로 조인하여 고른 음식 카테고리의 종류를 알아야한다고 생각하였고 음식 카테고리 테이블에서 식당으로 음식 카테고리 id로 조인하여 식당의 정보를 얻을 수 있다고 생각하였습니다.

WHERE의 mfc.member_id로 특정 회원의 선호 카테고리를 확인한다. 

회원-음식 카테고리에서 음식 카테고리를 기본 키로 조인하여 회원이 어떤 종류의 음식을 선호하는지를 알 수 있고 음식 카테고리와 식당을 조인하여 방금 선택된 카테고리의 식당 id로 식당을 알 수 있다.

이후 ORDER BY 를 사용해 식당의 평균 평점 내림차순, 리뷰 수 내림차순, 식당 id 오름차순 순서로 20개까지만 나타내게 된다.
